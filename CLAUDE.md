...
| Audit-log tamper-evidence (hash chain + `/audit/verify`) + SIEM export (HTTP/S3, flag-gated) | [backend/docs/audit-logging.md](backend/docs/audit-logging.md) + [docs/proposals/phase-16-audit-logging-controls.md](docs/proposals/phase-16-audit-logging-controls.md) |
...
- **Local-dev env is `.env.development` (committed), not `.env`.** Backend and frontend each ship a committed `backend/.env.development` / `frontend/.env.development` with safe local defaults (no secrets) so a fresh clone runs `pnpm dev` with **zero setup** — keep them secret-free. Machine-local overrides and real secrets go in a gitignored `.env.development.local` sibling (loaded last, so it wins); don't edit the committed file for personal tweaks (it dirties the tree) and don't reintroduce a `--env-file=.env` step. `.env` stays gitignored — it's the personal / SOPS-plaintext slot. **Exceptions that keep `.env.example`→`.env`:** `packages/flakey-cli/` and `examples/*` hold a real per-user `FLAKEY_API_KEY`, so they can't have a working committed default — leave them on the copy-and-fill flow. And **don't add `--env-file` to backend `seed`/`test`** — they run on code defaults on purpose so seeding gets the `flakey` superuser while the app runs as `flakey_app` (see [backend/CLAUDE.md](backend/CLAUDE.md)).
...
- **Folding a Dependabot / dependency-bump PR onto `main`: rebase first, then stage only the pin hunks.** A `chore(…)(deps)` commit must change *nothing* but the dependency pins it claims to (action SHAs, package versions, lockfiles). Before committing, rebase the fold onto current `main` and stage only the bump hunks (`git add -p path/to/file`) — never write a whole file out from the stale PR branch, which silently drags back any unrelated change made on `main` since the PR's base. (This is exactly how the e2e shard matrix got reverted 14→4 inside a commit titled "bump pinned action SHAs" — `e2be78b`.) After staging, `git diff --cached` and confirm every hunk is a pin change before committing.
...
The nine `@flakeytesting/*` packages publish to **npm** (auth via the `NPM_TOKEN` secret). The Python `flakey-pytest-reporter` publishes to **PyPI** from the same workflow but is on its **own version line** (`pyproject.toml`, currently `0.1.0`) and is **deliberately excluded from `all@`** — release it with a `pytest-reporter@<version>` tag (or the `pytest-reporter` `workflow_dispatch` choice). It builds with `uv build` and uploads via PyPI **Trusted Publishing (OIDC)** — no token secret. One-time PyPI setup is required before the first publish: add a pending publisher for project `flakey-pytest-reporter` → repo `Absence0760/project-flakey`, workflow `publish.yml`, environment `production` (see the comment above the `publish-pytest-reporter` job).

**Every publish job runs in the `production` GitHub Environment.** That environment scopes the `NPM_TOKEN` secret to publish jobs only and carries a **required-reviewer protection rule** — a release fires the workflow, but each publish waits in *pending* until an operator approves it in the Actions UI (the enforced "publishing is the operator's call" gate). The `production` environment name is also what the PyPI trusted-publisher config above must reference; don't rename it without updating PyPI in lockstep.
