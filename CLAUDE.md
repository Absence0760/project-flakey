...
| Deploying / production secrets | [docs/operations/deploy-plan.md](docs/operations/deploy-plan.md); secrets are auto-generated by default, optionally sops-sourced from the private `infra-secrets` repo — [docs/operations/secrets-sops.md](docs/operations/secrets-sops.md) |
...
- **Dependabot lockfile sync needs a PAT in the *Dependabot* secrets store.** `.github/workflows/dependabot-lockfile.yml` regenerates `pnpm-lock.yaml` + `frontend/pnpm-lock.yaml` on bot PRs (Dependabot's npm ecosystem only rewrites `package.json` on a pnpm project). It reads `DEPENDABOT_LOCKFILE_PAT` — which Dependabot-authored PRs can only see in the **Dependabot** store, not the Actions one (`gh secret set DEPENDABOT_LOCKFILE_PAT --app dependabot`). Without it the sync job now **fails** when a PR needs a sync, and warns when it doesn't; it used to skip silently and go green, which hid a completely inert workflow while #118 sat red for three weeks. `tests.yml`'s `workflow-consistency` job guards that fail-loud shape.
- **Two package managers.** `backend/` uses **npm** (its own lockfile, outside the pnpm workspace); `frontend/` + `packages/` use **pnpm**. Don't cross them. Lockfile authority: the root `pnpm-lock.yaml` owns every `packages/*` member, `backend/package-lock.json` owns the backend, and `frontend/` plus each `examples/*` is a standalone project with its own. A `pnpm-lock.yaml` inside a workspace member or inside `backend/` is drift — three survived a reverted April pnpm migration until 2026-08-20; `tests.yml`'s `consistency` job now fails on them.
