...
test("Jira settings are isolated across orgs (no plaintext, no cross-read)", async () => {
  ...
  // Parse the response and assert directly on the field — substring
  // includes() against a URL fragment trips CodeQL's
  // js/incomplete-url-substring-sanitization rule, and the structured
  // check is the more precise canary anyway (org B's base_url is
  // either exactly org A's value or it isn't).
  const parsedB = JSON.parse(dataB) as { has_api_token: boolean; jira_base_url?: string | null };
  assert.notEqual(parsedB.jira_base_url, "https://a.atlassian.net", "RLS leak: org B saw org A's base_url");
