...
router.post("/", async (req, res) => {
  try {
    const body = req.body as {
      ...
      raw?: unknown;
      // Or accept a pre-normalized payload directly
    ...
    if (body.raw && body.meta?.reporter) {
      // Raw reporter output — normalize it
...
// GET /runs/:id — single run with specs and tests
router.get("/:id", async (req, res) => {
  try {
    ...
    const specsResult = await pool.query(
      "SELECT * FROM specs WHERE run_id = $1 ORDER BY file_path",
      [runId]
    );
    ...
    if (specIds.length > 0) {
      const testsResult = await pool.query(
        "SELECT * FROM tests WHERE spec_id = ANY($1) ORDER BY id",
        [specIds]
      );
    ...
    const specs = specsResult.rows.map((spec) => ({
      ...spec,
      tests: tests[spec.id] ?? [],
    }));
