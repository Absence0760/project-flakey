...
| `045_encrypted_token_format_check.sql` | `NOT VALID` CHECK constraints requiring `organizations.git_token` / `jira_api_token` / `pagerduty_integration_key` to be `v1:` ciphertext — rejects plaintext token writes (makes `FLAKEY_ENCRYPTION_KEY` mandatory for integration secrets). Legacy plaintext rows tolerated until an operator backfills + `VALIDATE CONSTRAINT` |
