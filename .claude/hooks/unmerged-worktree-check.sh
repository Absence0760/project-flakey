...
${report}
\`main\` is sealed in this repo — no direct pushes, and merging needs a green
\`CI gate\` check (see the root CLAUDE.md). So this work lands via a PR, not a
local merge: rebase the branch on \`main\` to keep history linear, then push it
and open a PR. Pushing is the operator's call — offer, don't do it.
