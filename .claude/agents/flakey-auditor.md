...
| `api-contract` | Hand-synced route↔client drift (no codegen): an `api.ts` function whose URL/method/params don't match a real route; a response interface field the route's SELECT never returns (silent `undefined`); `*Detail` vs `*` shape mismatches; status-code handling (401 refresh / 403 / 404) vs what routes emit; `tests.status` enum union skew across `types.ts` / `api.ts` / DB CHECK; client `limit`/`runs`/`window` params vs server caps (`flaky.ts`). Separate from `migrations` (DB-column drift) — this is the request/response contract | `backend/src/index.ts`, `backend/src/routes/*.ts`, `frontend/src/lib/api.ts`, `backend/src/types.ts` |
| `seed-integrity` | e2e-flake fragility seeded into the data: fixtures parked at the `PAGE_SIZE=50` client-pagination boundary (`v2.5.0` release rank ~50; `/flaky` candidate count near 50 within the default 30-run window); non-deterministic per-tenant ordering (sort with no stable tiebreak); additive re-seed pollution; worker-tenant (`acme-w{0..3}`) asymmetry; randomness that can empty a page | `backend/src/seed.ts`, `frontend/tests-e2e/` (esp. `cross-cutting/pagination.spec.ts`, `releases/`), the `/flaky` + `/releases` route pagination |
| `migration-locks` | Prod-DDL safety: table-rewriting / `ACCESS EXCLUSIVE` ops against a populated RDS — `ADD COLUMN` with volatile default, `ALTER TYPE`, `ADD CONSTRAINT`/`FK` without `NOT VALID`+`VALIDATE`, plain `CREATE INDEX` vs `CONCURRENTLY`, in-migration backfills; the `CONCURRENTLY`-can't-run-in-a-txn gotcha vs `migrate.sh`. Separate from `migrations` (idempotency/RLS/type-drift) — this is lock impact only | `backend/migrations/*.sql`, `backend/migrate.sh`, `020_performance_indexes.sql` |
