# Flakey — Postman example

Runs a Postman collection via Newman against the Flakey backend and
...
| Flaky-test detection | The **Flaky** folder in `collection.json` contains a request with an intentional ~30% failure (`Math.random() < 0.3`). Run `test:smoke` repeatedly and watch Flakey surface the instability |
