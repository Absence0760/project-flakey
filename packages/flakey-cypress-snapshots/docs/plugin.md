...
3. **Gherkin step markers (Cucumber)** — automatic from the `./support` import. The `command:end` handler reads `@badeball`'s active pickle step (`window.testState.pickleStep`); when it changes, `markGherkinStep` pushes a synthetic step with `commandName: "gherkin"` and `commandMessage: "Given/When/Then <text>"`. The viewer renders these with distinct styling so Cypress commands group visually under their scenario step. The optional `./cucumber` subpath registers a `BeforeStep` hook that calls the same `markGherkinStep` (deduped by step id) — its only benefit is firing *before* the step's first command instead of at it; no extra import is needed just to get grouping.
