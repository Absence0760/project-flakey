...
test.describe("issue #41 — live runs surface in the runs list mid-flight", () => {
  ...
  test("a fresh /live/start run appears as a row with the LIVE badge without reloading", async ({
    ...
    await expect(
      newRow.locator(".live-badge"),
      "the new row must show the LIVE badge",
    ).toBeVisible({ timeout: 10_000 });

    await deleteRun(page, token, runId);
  });

  test("two parallel /live/start runs both surface as rows with LIVE badges (cardinality / no dedup)", async ({
    page,
  }) => {
    ...
    const rowA = page.locator(`tr.run-row[data-run-id="${runIdA}"]`);
    const rowB = page.locator(`tr.run-row[data-run-id="${runIdB}"]`);
    await expect(rowA, "first parallel live run must appear").toBeVisible({ timeout: 25_000 });
    await expect(rowB, "second parallel live run must appear").toBeVisible({ timeout: 25_000 });
    await expect(rowA.locator(".live-badge")).toBeVisible({ timeout: 10_000 });
    await expect(rowB.locator(".live-badge")).toBeVisible({ timeout: 10_000 });
  ...
  test("a live run aborted mid-flight drops its LIVE badge in the list (abort path, distinct from clean finish)", async ({
    ...
    await expect(row.locator(".live-badge")).toBeVisible({ timeout: 10_000 });
    ...
    await expect(row.locator(".live-badge")).toHaveCount(0, { timeout: 20_000 });
    await expect(row, "aborted run must still be visible in the list").toBeVisible();
  ...
  test("a run that finishes between polls still updates the card (regression guard for the existing 'removal' branch)", async ({
    ...
    // Wait for the new row + LIVE badge to render.
    const row = page.locator(`tr.run-row[data-run-id="${runId}"]`);
    await expect(row).toBeVisible({ timeout: 25_000 });
    await expect(row.locator(".live-badge")).toBeVisible({ timeout: 10_000 });
